...
def fetch_kma_past_asos_wind(start_date, end_date, auth_key):
    ...
    print(f"  [ASOS Wind] {len(station_dfs)}개 관측소 가중평균 완료 ({zone_names}), {len(df_result)}행")
    return df_result
...
def fetch_kma_future_ncm(lat, lon, auth_key, base_date_kst, as_of_kst=None):
    ...
    if not candidates:
        print(f"  [KMA NCM] 사용 가능한 예보 사이클 없음")
        return pd.DataFrame()
    ...
    print(f"  [KMA NCM] 예보 기준: {base_tmfc} UTC "
    ...
    if not target_hours:
        print(f"  [KMA NCM] 수집 대상 시간 없음")
        return pd.DataFrame()

    print(f"  [KMA NCM] 수집 범위: "
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    wait = 2 ** (attempt + 1)
                    print(f"    [Retry] +{hour_offset}h: {attempt+1}/{max_retries} ({wait}s 대기)")
                    time.sleep(wait)
                else:
                    print(f"    [Fail] +{hour_offset}h: {max_retries}회 시도 실패")
        return None
    ...
    if not rows:
        ...
        if candidates:
            ...
            print(f"  [KMA NCM] 데이터 없음 → 이전 사이클 {fallback_tmfc} UTC로 재시도")
    ...
    print(f"  [KMA NCM] {len(df)}행 수집 완료 (예보 기준: {base_tmfc} UTC)")
    return df
...
def fetch_kma_future_ncm_north(lat, lon, auth_key, base_date_kst, as_of_kst=None):
    ...
    print(f"  [KMA NCM north] {len(df)}행 수집 완료")
    return df
...
def fetch_kma_future_ncm_wind(auth_key, base_date_kst, as_of_kst=None):
    ...
    print(f"  [Wind] {actual_zones}개 권역 가중평균 완료 ({zone_names}), {len(df_result)}행")
