...
def fetch_kpx_past(start_date, end_date):
    """KPX 제주 수급현황에서 실측 수요(MW)를 가져온다 (인증키 불필요).

    start_date/end_date: 'YYYY-MM-DD' (하이픈 포함).
    반환: timestamp 인덱스(1시간 단위) + real_demand 컬럼.
    """
    ...
    try:
        ...
        df = df.rename(columns={'현재수요(MW)': 'real_demand'})
        result = df.set_index('timestamp')[['real_demand']].apply(pd.to_numeric, errors='coerce')

        # demand=0 은 계측 오류 (실제 수요가 0이 될 수 없음) → NaN 처리 후 양방향 보간(최대 3개 연속)
        zero_mask = result['real_demand'] == 0
        if zero_mask.any():
            result.loc[zero_mask, 'real_demand'] = np.nan
