...
def daily_historical_update(start_date, end_date):
    ...
    try:
        ...
        if not asos_south.empty and not asos_wind.empty:
            ...
            asos_data = asos_south
            print("  [Warning] 풍속 2권역 데이터 없음 — 남쪽 ASOS만 사용")
        else:
            asos_data = pd.DataFrame()
...
def daily_forecast_and_predict(start_date, end_date):
    ...
    while current_dt <= end_dt:
        ...
        try:
            if target_d < today:
                # ── 과거: 사이클 시간순 소급 ──
                print(f"  └── [KMA] 과거 소급 수집 ({len(CYCLE_AVAILABLE_KST)} 사이클)")
                ...
                if not kma_data.empty:
                    db.save_forecast(kma_data, auto_add_capacity=True)
                    print(f"  └── [KMA] 오늘 멀티사이클 저장 완료")
                else:
                    print(f"  └── [KMA] 최신 사이클 데이터 없음")
                ...
                if not kma_data.empty:
                    db.save_forecast(kma_data, auto_add_capacity=True)
                    print(f"  └── [KMA] {len(kma_data)}행 저장 완료")
                else:
                    print(f"  └── [KMA] 데이터 없음")
...
def daily_forecast_kma(start_date, end_date):
    ...
    print(f"[Start] KMA 예보 수집: {start_date} ~ {end_date}")
    db = JejuEnergyDB()
    current_dt = start_dt
    ...
    while current_dt <= end_dt:
        ...
        try:
            if target_d < today:
                # ── 과거 날짜: 사이클 시간순 소급 호출 ──
                print(f"[{target_date}] 과거 소급 수집 ({len(CYCLE_AVAILABLE_KST)} 사이클)")
                for time_str in CYCLE_AVAILABLE_KST:
                    as_of = f"{target_date} {time_str}"
                ...
                print(f"[{target_date}] [Done] 과거 소급 완료")
                
            elif target_d == today:
                # ── 오늘: 이미 지난 사이클 순차 호출 + 최신 사이클 ──
                print(f"[{target_date}] 오늘 - 멀티사이클 수집")
                for time_str in CYCLE_AVAILABLE_KST:
                    cycle_time = datetime.strptime(
                        f"{target_date} {time_str}", "%Y-%m-%d %H:%M"
                    ).replace(tzinfo=KST)
                ...
                if not kma_data.empty:
                    db.save_forecast(kma_data, auto_add_capacity=True)
                    print(f"[{target_date}] [Success] 오늘 멀티사이클 저장 완료")
                else:
                    print(f"[{target_date}] [Fail] 최신 사이클 데이터 없음")
                    
            else:
                # ── 미래: 현재 최신 사이클로 가능한 만큼 ──
                print(f"[{target_date}] 미래 - 현재 최신 사이클 호출")
                ncm_south = fetch_kma_future_ncm(
                    33.3284, 126.8366, KMA_KEY, target_date)
                ncm_north = fetch_kma_future_ncm_wind(
                    KMA_KEY, target_date)
                ...
                if not kma_data.empty:
                    db.save_forecast(kma_data, auto_add_capacity=True)
                    print(f"[{target_date}] [Success] {len(kma_data)}행 저장")
                else:
...
def _merge_south_north(ncm_south, ncm_north):
    ...
    if not ncm_south.empty and not ncm_north.empty:
        ...
        print(f"  [Warning] 북쪽 NCM 없음, 남쪽만 사용")
        return ncm_south
    return pd.DataFrame()
