...
# 서빙 안전망: DB에 이미 시간 구멍이 있어도 예측이 돌아가도록 채우는 연속 결측 허용 개수
_SERVING_INTERP_LIMIT = 3


def fill_hourly_gaps(df, start_str, end_str):
    """
    모델 입력을 1시간 간격 격자로 되돌린다.

    예보가 3시간 간격으로 들어와 DB에 시간 구멍이 남은 구간에서도 예측이
    돌아가게 하는 안전망. 보간 결과는 메모리에만 존재하며 DB에는 쓰지 않는다.
    """
    if df.empty:
        return df

    full_index = pd.date_range(start=start_str, end=end_str, freq='h')
    full_index_str = full_index.strftime('%Y-%m-%d %H:%M:%S')

    missing_count = len(full_index_str.difference(df.index))
    if missing_count == 0:
        return df
    ...
    filled.index = full_index_str
    filled.index.name = df.index.name

    logger.warning(f"모델 입력에 {missing_count}개 시각 누락 → 보간으로 채움 "
                   f"(DB 미반영, {len(df)}행 → {len(filled)}행)")
    return filled
...
def run_model_prediction(target_date, db, assets):
    ...
    # 예보가 3시간 간격으로 들어온 구간의 시간 구멍 보정 (메모리에서만)
    df = fill_hourly_gaps(df, start_str, end_str)
