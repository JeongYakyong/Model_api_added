"""sync_forecast.py — jeju_model 의 예측(수요·순부하·신재생)을 읽어와 자체 DB에 복사하는 cron 진입점.
...
def fetch_freshest_forecast(retries=3, retry_wait=2):
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            try:
                df = pd.read_sql_query(FRESHEST_SQL, con, params=(HORIZON_MIN, HORIZON_MAX, start, end))
            finally:
                con.close()
            return df
